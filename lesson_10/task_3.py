...
class Cell:
    ...
    def __add__(self, other):
        return __class__(self.items + other.items)

    def __sub__(self, other):
        if self.items == other.items:
            raise ValueError('Ошибка вычитания: количества ячеек операндов не должны совпадать!')
        return __class__(abs(self.items - other.items))
    ...
    def __floordiv__(self, other):
        if self.items == other.items:
            raise ValueError('Ошибка деления: количества ячеек операндов не должны совпадать!')
        return __class__(max(self.items, other.items) // min(self.items, other.items))

    def make_order(self, count=5):
        result = ['⚪' * count for _ in range(self.items // count)]
        result.append('⚪' * (self.items % count))
        return '\n'.join(result)
...
if __name__ == '__main__':
    ...
    print('+', (cell_1 + cell_2 + cell_3).make_order(20))
    print('-', (cell_1 - cell_2 - cell_3).make_order(20))
    print('*', (cell_1 * cell_2 * cell_3).make_order(20))
    print('//', (cell_1 // cell_2 // cell_3).make_order(20))
